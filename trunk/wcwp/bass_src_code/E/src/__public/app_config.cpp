...
TCString TAppPathLinks::GetRunBinPath()
{
    ...
    if (STA__sRunBinPath == TCString("__UNKW__") )
    {
        if (!PathLinkExists())
            STA__sRunBinPath = "";
        else
...
long TAppPathLinks::GetSuperConfigPathsCount()
{
    ...
    for (i = 1; i <= SUPER_CONFIG_PATH_MAX_COUNT; i++)
    {
        ...
        if (STA__sAPLSuperConfigPaths[i - 1] != TCString("") )
        {
...
{
    static bool STA__bRunFirstTime = true;
    static bool STA__bPathLinkExists = false;

    if (STA__bRunFirstTime)
    {
        STA__bRunFirstTime = false;
...
TCString TAppPath::AppTemp(TCString sSubDirName)
{
    ...
    sRet = MergePathAndFile(AppRoot(), "mid_files");
...
printf("MI_TEMP_DIR\n");
    ...
    sRet = pStr;
    sRet = IncludeTrailingSlash(sRet);
#endif
*/
    if (sSubDirName == TCString("") )
        sSubDirName = Application.GetAppName();

    sRet = IncludeTrailingSlash(MergePathAndFile(sRet, sSubDirName));
...
TCString TAppPath::AppRoot()
{
    ...
    if (STA__sAppRoot == TCString("__UNKW__") )
    { 
#ifdef __WIN32__
...
{
    if (sParmName == TCString("未漫游基本通话费") )
         return "600";

    if (sParmName == TCString("漫游基本通话费") )
        return "800";

    if (sParmName == TCString("配置文件目录") )
        return TAppPath::AppConfig();

    if (sParmName == TCString("临时文件目录") )
    {
        static TCString STA__TempDir = "__UNKW__";
        if (STA__TempDir == TCString("__UNKW__") )
    ...
    if (sParmName == TCString("采集机通用配置") )
        return TAppFile::GatherGeneralConfig();

    if (sParmName == TCString("采集机数据目录") )
    {
        static TCString sWorkDataDir;
        if (sWorkDataDir == TCString("") )
    ...
    if (sParmName == TCString("采集机停止运行空间百分比") )
    {
        static TCString s_sLeastFreePercent;

        if (s_sLeastFreePercent == TCString("") )
    ...
    if (sParmName == TCString("采集机暂停运行空间百分比") )
    {
        static TCString s_sPauseFreePercent;

        if (s_sPauseFreePercent == TCString("") )
    ...
    if (sParmName == TCString("数据库服务名称") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DATABASE", "ServerName", "");

    if (sParmName == TCString("数据库登录用户") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DATABASE", "LogID", "");

    if (sParmName == TCString("数据库登录口令") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DATABASE", "LogPassword", "");

    if (sParmName == TCString("数据库数据库名") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DATABASE", "Database", "");

    /*add begin by zhoulm in 20030421 for connect sales database*/
    if (sParmName == TCString("SalesServerName") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseSale", "ServerName", "");

    if (sParmName == TCString("SalesLogID") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseSale", "LogID", "");

    if (sParmName == TCString("SalesPasswd") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseSale", "LogPassword", "");

    if (sParmName == TCString("SalesDB") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseSale", "Database", "");

    /*===============add begin by zhoulm in 20030623 for connect sales database*/
    if (sParmName == TCString("AccountServerName") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseAccount", "ServerName", "");

    if (sParmName == TCString("AccountLogID") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseAccount", "LogID", "");

    if (sParmName == TCString("AccountPasswd") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseAccount", "LogPassword", "");

    if (sParmName == TCString("AccountDB") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseAccount", "Database", "");

    if (sParmName == TCString("ParaServerName") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabasePara", "ServerName", "");

    if (sParmName == TCString("ParaLogID") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabasePara", "LogID", "");

    if (sParmName == TCString("ParaPasswd") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabasePara", "LogPassword", "");

    if (sParmName == TCString("ParaDB") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabasePara", "Database", "");

    // add by zhoulm in 20030102   MainCustServerName
    if (sParmName == TCString("MainCustServerName") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseMainCust", "ServerName", "");

    if (sParmName == TCString("MainCustLogID") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseMainCust", "LogID", "");

    if (sParmName == TCString("MainCustPasswd") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseMainCust", "LogPassword", "");

    if (sParmName == TCString("MainCustDB") )
        return ProfileString(TAppFile::GatherGeneralConfig(),
                "DatabaseMainCust", "Database", "");
