...
// 函数 : RefreshAllEnv
// 用途 : 刷新所有的环境配置
// 原型 : void void RefreshAllEnv();
// 参数 : 无
// 返回 : 无
// 说明 : mig_ctrl在程序中改变环境变量时调用
//==========================================================================
void RefreshAllEnv()
{

    STA__sRunBinPath = "__UNKW__";

    STA__nAPLSuperConfigPathsCount = -1;               // 配置目录的数量

    STA__bPathLinkExistsRunFirstTime = true;

    STA__sAppTempRoot = "__UNKW__";
    STA__sLastAppTempReturn = "__UNKW__";

    STA__bAppLogRunFirstTime = true;
    STA__bAppRunningInfoRunFirstTime = true;

    STA__sAppRoot = "__UNKW__";

    STA__sLastAppName = "__UNKW__";
    STA__sLastAppConfigFileName = "__UNKW__";

    STA__sGeneralConfigFileName = "__UNKW__";
}
