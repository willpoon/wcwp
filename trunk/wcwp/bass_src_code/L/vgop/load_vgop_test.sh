#!/bin/ksh
#*******************************************************
#函数名：load_vgop_test.sh
#功  能：获取VGOP接口信息，并加载入库
#参  数： 
#author: zhaolp2      
#日  期: 2010-09-02	      
#输  出: 此shell为VGOP接口联调测试使用
#*******************************************************
...
if [ $# -eq 1 ] ; then
   filename=$1
	...
	configFlag=`grep ${task_id} ${WORK_PATH}/load_common.cfg |grep -v "#"| wc -l`
	...
	control_code=TR1_${task_id}
	#通过配置文件获取模板表名称和接口中文描述
	echo $control_code
	table_name_templet=`grep -i ${task_id} ${WORK_PATH}/load_common.cfg|grep -v "#"| awk -F',' '{print $2}'`
	task_name=`grep -i ${task_id} ${WORK_PATH}/load_common.cfg |grep -v "#"| awk -F',' '{print $3}'`
	...
	##如果接口文件存在，执行加载动作
	#if [ -f ${BACKUP_PATH}/$filename -o -f ${BACKUP_PATH}/${time_id}/$filename ]; then
	#
	#	cp ${BACKUP_PATH}/${time_id}/$filename ${WORK_PATH}/
	#	cp ${BACKUP_PATH}/${time_id}/$chkfilename ${WORK_PATH}/
	#	cp ${BACKUP_PATH}/$filename ${WORK_PATH}/
	#	cp ${BACKUP_PATH}/$chkfilename ${WORK_PATH}/
	#	
	#	file_rows_fact=`wc -l ${WORK_PATH}/${filename}|awk '{print $1}'`
	#	
	#	cd ${WORK_PATH}	
  #
	#	DB2_SQLCOMM="db2 \"drop table bass2.${table_name}\""
	#        DB2_SQL_EXEC 
	#        
	#	DB2_SQLCOMM="db2 \"create table bass2.$table_name like bass2.$table_name_templet in tbs_ods_other index in tbs_index partitioning key (ROW_NO) using hashing not logged initially\""
	#        DB2_SQL_EXEC 
  #
  #
	...
	#	
	#	DB2_SQLCOMM="db2 \"load client from ${WORK_PATH}/${filename} of del modified by coldel$ fastparse anyorder warningcount 1000 messages /dev/null replace into ${table_name} \""
	#	echo $DB2_SQLCOMM;
	#	DB2_SQL_EXEC
	#	
	...
	#		DB2_SQL_EXEC  > /dev/null
	#		DB2_SQLCOMM="db2 \"insert into APP.SCH_CONTROL_ALARM(CONTROL_CODE,CMD_LINE,GRADE,CONTENT,ALARMTIME,FLAG) values('${control_code}','load_vgop.sh',1,'${ALARM_CONTENT}',current timestamp,-1)\""
	#		DB2_SQL_EXEC  > /dev/null
	#	fi		
	#	
	#fi
