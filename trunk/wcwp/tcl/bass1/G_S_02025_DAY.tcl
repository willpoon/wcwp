...
######################################################################################################		
#接口名称: "全球通专属叠加资费套餐用户成功办理量"                                                               
#接口编码：02025                                                                                          
#接口说明："上报当日所有成功办理的全球通全网统一专属叠加资费套餐的订购关系"
#程序名称: G_S_02025_DAY.tcl                                                                            
#功能描述: 生成02025的数据
#运行粒度: DAY
#源    表：1.
#输入参数: void
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
    ...
    puts $last_month
        ...
        #程序名
        global app_name
        set app_name "G_S_02025_DAY.tcl"
	
  #删除本期数据
	set sql_buff "delete from bass1.G_S_02025_DAY where time_id=$timestamp"
	exec_sql $sql_buff

	set sql_buff "ALTER TABLE BASS1.G_S_02025_DAY_1 ACTIVATE NOT LOGGED INITIALLY WITH EMPTY TABLE"
	exec_sql $sql_buff
	
  #直接来源于二经用户表数据，新的接口表
	set sql_buff "
		
insert into bass1.G_S_02025_DAY_1
select 
	$timestamp time_id
...
exec_sql $sql_buff
  ...
  #进行结果数据检查
  #1.检查chkpkunique
	set tabname "G_S_02025_DAY"
	set pk 			"user_id||ADD_PKG_ID"
	chkpkunique ${tabname} ${pk} ${timestamp}
  
  #2.检查在网用户是否在用户表里头 & 品牌不要求一定为全球通
...
#	set sql_buff "select count(*) from 
#	            (
#		     select distinct ADD_PKG_ID from bass1.G_S_02025_DAY
#		      where time_id =$timestamp
#		       except
...
#	set sql_buff "select count(*) from 
#	            (
#		     select distinct CHANNEL_ID from bass1.G_S_02025_DAY
