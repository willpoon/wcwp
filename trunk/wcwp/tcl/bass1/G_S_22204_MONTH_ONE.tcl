...
#程序名称: g_s_22204_month_one.tcl
#功能描述: 每个月生成TD客户指标以及收入,完成一个月数据的抓取，插入中间表 g_s_22204_month_tmp3
#运行粒度: 月
#源    表：1.
#          2.
#输入参数:
...
##set last_last_month_day  "20100501"
	...
	set sql_buff "delete from bass1.g_s_22204_month_tmp3 where cycle_id='1'"
    ...
    exec db2 runstats on table bass1.td_check_user_status_ls with distribution and detailed indexes all
...
#-----------------------------------------------------------------------------------
	...
	set sql_buff "
		...
		values('1',decimal(1.0*($RESULT_VAL1+$RESULT_VAL2-$RESULT_VAL3)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('1',decimal(1.0*($RESULT_VAL4+$RESULT_VAL5-$RESULT_VAL6)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('1',decimal(1.0*($RESULT_VAL7+$RESULT_VAL8)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('1',decimal(1.0*($RESULT_VAL9+$RESULT_VAL10)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('1',decimal(1.0*($RESULT_VAL11+$RESULT_VAL12+$RESULT_VAL13)/100,20,2))
	  "
