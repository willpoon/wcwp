...
########################################################################################################3
	# R005	月	集团统付账单接口的集团客户标识应存在于集团客户接口中
  ...
  set handle [aidb_open $conn]
   ...
   puts $sql_buff
	...
	set handle [aidb_open $conn]
	set sql_buff "\
		INSERT INTO BASS1.G_RULE_CHECK VALUES ($op_month,'R005',$DEC_RESULT_VAL1,0,0,0) "
	if [catch { aidb_sql $handle $sql_buff } errmsg ] {
	...
	#--判断
	#--异常
	#--1：集团统付账单接口的集团客户标识应存在于集团客户接口中


	if {[format %.3f [expr ${DEC_RESULT_VAL1} ]]>0 } {
		set grade 2
	        set alarmcontent " R005校验不通过,03013统付接口的集团客户标识不在集团客户表中"
