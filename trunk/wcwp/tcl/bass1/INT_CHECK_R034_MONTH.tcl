#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R034_MONTH.tcl
# ** 程序功能：一经校验程序（R034）
# ** 运行示例：int INT_CHECK_R034_MONTH.tcl
# ** 创建时间：2009-5-29 15:33:38
# ** 创 建 人：xufr
# ** 规则编号：R034
# ** 规则属性：业务逻辑
# ** 规则类型：月
...
# **  All Rights Reserved.
# **  20091203 根据1.6.3规范，取消当月收费用户数，因此这个规则无效
#***********************************************************************************************************************************************
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
##	set app_name "INT_CHECK_R034_MONTH.tcl"
##
##	#删除本期数据
##	set handle [ aidb_open $conn ]
##	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R034'"
##	puts ${sqlbuf}
...
##	#当月收费用户数
##	set handle [ aidb_open $conn ]
##	set sqlbuf "select int(m_bill_users) from bass1.g_s_22012_day where time_id  = ${op_month_last}"
##    puts ${sqlbuf}
##	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
...
##	#帐单用户数
##	set handle [ aidb_open $conn ]
##	set sqlbuf "select count(distinct user_id) from bass1.g_s_03004_month where time_id = ${op_month}"
##    puts ${sqlbuf}
##	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
...
##	aidb_close $handle
##
##	#判断R034：当月收费用户数<=帐单用户数
##	if { $RESULT_VAL1 > $RESULT_VAL2 } {
##		set grade 2
##		set alarmcontent "准确性指标R034超出集团考核范围"
