#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R041_MONTH.tcl
# ** 程序功能：一经校验程序（R041）
# ** 运行示例：int INT_CHECK_R041_MONTH.tcl
# ** 创建时间：2009-5-31 9:43:23
# ** 创 建 人：xufr
# ** 规则编号：R041
# ** 规则属性：业务逻辑
# ** 规则类型：月
# ** 指标摘要：实体渠道月汇总接口中的渠道标识
# ** 规则描述：实体渠道月汇总接口中的渠道标识都应该在实体渠道接口中
# ** 校验对象：06030 实体渠道
# **           22054 实体渠道月汇总
# ** 输出参数：返回值:   0 成功; -1 失败
# ** 修改历史:
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	puts $op_month_last
	#程序名称
	set app_name "INT_CHECK_R041_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R041'"
	puts ${sqlbuf}
	...
	#判断实体渠道月汇总接口在实体渠道接口中不存在的记录数
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select count(*) from (
	select value(channel_id,'0') from bass1.g_s_22054_month where time_id = $op_month
	except
	select value(channel_id,'0') from bass1.G_A_06030_DAY where time_id <=${op_month_last} group by channel_id) a"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	if { $RESULT_VAL1 > 0 } {
		set grade 2
		set alarmcontent "准确性指标R041超出集团考核范围"
