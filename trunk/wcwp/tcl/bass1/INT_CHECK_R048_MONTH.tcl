#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R048_MONTH.tcl
# ** 程序功能：一经校验程序（R048）
# ** 运行示例：int INT_CHECK_R048_MONTH.tcl
# ** 创建时间：2009-5-31 9:45:46
# ** 创 建 人：xufr
# ** 规则编号：R048
# ** 规则属性：业务逻辑
# ** 规则类型：月
# ** 指标摘要：社会渠道的分业务办理笔数和分品牌办理笔数的平衡关系
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R048_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R048'"
	puts ${sqlbuf}
	...
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select value(sum(int(NEWCUSTCOUNT) + int(PAYCOUNT) + int(BUSICOUNT) + int(COMINALCOUNT) + int(OTHERCOUNT)),0) from bass1.G_S_22047_MONTH where time_id = $op_month"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	if { $RESULT_VAL1 != $RESULT_VAL2 } {
		set grade 2
		set alarmcontent "准确性指标R048超出集团考核范围"
