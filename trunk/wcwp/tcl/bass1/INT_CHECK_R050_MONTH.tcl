#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R050_MONTH.tcl
# ** 程序功能：一经校验程序（R050）
# ** 运行示例：int INT_CHECK_R050_MONTH.tcl
...
# ** 规则类型：月
# ** 指标摘要：实体渠道缴费笔数
# ** 规则描述：实体渠道月汇总接口中缴费笔数应等于自营渠道业务量情况接口和社会渠道业务情况接口中缴费笔数之和
# ** 校验对象：22054 实体渠道月汇总
# **           22046 自营渠道业务量情况
# **           22047 社会渠道业务情况
# ** 输出参数：返回值:   0 成功; -1 失败
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R050_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R050'"
	puts ${sqlbuf}
	...
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select value(sum(int(chargecount)),0) from bass1.G_S_22054_MONTH where time_id = $op_month"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	#自营渠道业务量情况接口缴费笔数
	set handle [ aidb_open $conn ]
	set sqlbuf "select value(sum(int(paycount)),0) from bass1.G_S_22046_MONTH where time_id = $op_month"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	#社会渠道业务情况接口缴费笔数
	set handle [ aidb_open $conn ]
	set sqlbuf "select value(sum(int(paycount)),0) from bass1.G_S_22047_MONTH where time_id = $op_month"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	#判断R050：实体渠道月汇总接口中缴费笔数 = 自营渠道业务量情况接口缴费笔数 + 社会渠道业务情况接口缴费笔数
	if { [ expr $RESULT_VAL2 + $RESULT_VAL3 ] != $RESULT_VAL1 } {
		set grade 2
		set alarmcontent "准确性指标R050超出集团考核范围"
