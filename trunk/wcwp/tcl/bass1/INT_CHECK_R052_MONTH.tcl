#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R052_MONTH.tcl
# ** 程序功能：一经校验程序（R052）
# ** 运行示例：int INT_CHECK_R052_MONTH.tcl
# ** 创建时间：2009-5-31 9:45:46
# ** 创 建 人：xufr
# ** 规则编号：R052
# ** 规则属性：业务逻辑
# ** 规则类型：月
# ** 指标摘要：实体渠道月汇总接口中业务办理量
# ** 规则描述：实体渠道月汇总接口中业务办理量应等于自营渠道业务量情况接口和社会渠道业务情况接口中全球通用户业务办理笔数+神州行用户业务办理笔数+动感地带用户业务办理笔数之和
# ** 校验对象：22054 实体渠道月汇总
# **           22046 自营渠道业务量情况
# **           22047 社会渠道业务情况
# ** 输出参数：返回值:   0 成功; -1 失败
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R052_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R052'"
	puts ${sqlbuf}
	...
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select value(sum(int(opercount)),0) from bass1.G_S_22054_MONTH where time_id = $op_month"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	#自营渠道业务量情况接口全球通用户业务办理笔数+神州行用户业务办理笔数+动感地带用户业务办理笔数
	set handle [ aidb_open $conn ]
	...
	#社会渠道业务情况接口全球通用户业务办理笔数+神州行用户业务办理笔数+动感地带用户业务办理笔数
	set handle [ aidb_open $conn ]
	set sqlbuf "select value(sum(int(QQTOPERATORCOUNT) + int(SZXOPERATORCOUNT) + int(DGDDOPERATORCOUNT)),0) from bass1.G_S_22047_MONTH where time_id = $op_month"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	#判断R052：实体渠道月汇总接口中业务办理笔数 = 自营渠道业务量情况接口品牌业务办理量之和 + 社会渠道业务情况接口品牌业务办理量之和
	if { [ expr $RESULT_VAL2 + $RESULT_VAL3 ] != $RESULT_VAL1 } {
		set grade 2
		set alarmcontent "准确性指标R052超出集团考核范围"
