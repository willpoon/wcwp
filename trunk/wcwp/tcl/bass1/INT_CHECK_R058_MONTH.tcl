#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R058_MONTH.tcl
# ** 程序功能：一经校验程序（R058）
# ** 运行示例：int INT_CHECK_R058_MONTH.tcl
...
# ** 规则类型：月
# ** 指标摘要：SP业务代码的组合唯一性
# ** 规则描述：SP业务代码表中有效的[SP企业代码、SP业务代码、计费类型]组合唯一
# ** 校验对象：06029 SP业务代码
# ** 输出参数：返回值:   0 成功; -1 失败
# ** 修改历史:
# **           修改日期       修改人        修改内容
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R058_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R058'"
	puts ${sqlbuf}
	...
	set sqlbuf "
	select count(*) from (
	select sp_code,operator_code,sp_bill_id from bass1.g_i_06029_month
	where time_id = $op_month
	group by sp_code,operator_code,sp_bill_id
	having count(*) > 1 ) a"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	if { $RESULT_VAL1 > 0 } {
		set grade 2
		set alarmcontent "准确性指标R058超出集团考核范围"
