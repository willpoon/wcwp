#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R060_MONTH.tcl
# ** 程序功能：一经校验程序（R060）
# ** 运行示例：int INT_CHECK_R060_MONTH.tcl
# ** 创建时间：2009-5-30 10:38:56
# ** 创 建 人：xufr
# ** 规则编号：R060
# ** 规则属性：业务逻辑
# ** 规则类型：月
# ** 指标摘要：SP结算表中CMCC运营机构标识
# ** 规则描述：SP结算表中CMCC运营机构标识应该属于该省的地市
# ** 校验对象：05003 SP结算
# ** 输出参数：返回值:   0 成功; -1 失败
# ** 修改历史:
# **           修改日期       修改人        修改内容
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R060_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R060'"
	puts ${sqlbuf}
	...
	#判断是否SP结算表中CMCC运营机构标识属于该省的地市
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select count(*) from (
	select distinct cmcc_code from bass1.g_s_05003_month where time_id = $op_month
	except
	select bass1_value from bass1.all_dim_lkp where bass1_tbid = 'BASS_STD1_0054') a"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	if { $RESULT_VAL1 > 0 } {
		set grade 2
		set alarmcontent "准确性指标R060超出集团考核范围"
