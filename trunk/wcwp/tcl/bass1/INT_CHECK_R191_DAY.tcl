...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
        ...
        #程序名
        set app_name "INT_CHECK_R191_DAY.tcl"



########################################################################################################3

#R191			新增	月	02_集团客户	移动400业务订购情况接口中的集团客户标识存在于集团客户资料中	02064(移动400业务订购情况)中的"集团客户标识"都在01004(集团客户)的"集团客户标识"中	0.05		


 	  set sqlbuf "delete from  BASS1.G_RULE_CHECK where time_id=$timestamp and rule_code in ('R191') "        
...
LEFT JOIN bass1.dim_trans_enterprise_id B on  A.enterprise_id = B.ENTERPRISE_ID  ) t where a.enterprise_id = t.enterprise_id )
  ...
  #获得结果值
	...
	#--将校验值插入校验结果表
	set sql_buff "
		INSERT INTO BASS1.G_RULE_CHECK VALUES ($timestamp,'R191',$RESULT_VAL,0,0,0) 
		"
	exec_sql $sql_buff
 	#检查合法性: 0 - 正常； 大于0 - 非正常
	if {[format %.3f [expr ${RESULT_VAL} ]]>0 } {
		set grade 2
	        set alarmcontent " R191 校验不通过"
	        puts ${alarmcontent}
