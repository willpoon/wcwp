      ...
      left join
           (
            select enterprise_id,
                   ...
                   sum(non_unipay_fee) ent_non_income
  ...
  #循环结束
    ...
    select city_id,
     ...
     where time_order = 1
       ...
       and (ent_prod_income+ent_non_income) > 0
     group by
           city_id,
           case when ent_prod_counts = 1 then 18
                when ent_prod_counts = 2 then 19
    ...
    select b.city_id,
      ...
      from (select enterprise_id
                   ,sum(member_income+ent_prod_income) as total_income
                   ,sum(ent_prod_income+ent_non_income) as prod_income
            from $stat_enterprise_0060_a_tmp1
            where time_order in (1,2,3)
            group by enterprise_id
            ) a,
            $stat_enterprise_0060_a_tmp1 b
     where a.total_income > 0
       and a.prod_income = 0
       and b.ent_sub_counts > 0
       and b.time_order = 1
       and a.enterprise_id = b.enterprise_id
     group by
           b.city_id;"
    ...
    select 2,
     ...
     where a.enterprise_id = b.enterprise_id
       and a.dummy_mark = 0
       and b.ent_status_id = 0
       and b.enterprise_id not in (select enterprise_id from $dw_enterprise_msg_lmyyyymm  where is_test = 1);"
    puts $sql_buf
    if [catch {aidb_sql $handle $sql_buf} errmsg] {
    ...
    select case when a.level_def_mode = 1 then '888'
     ...
     where a.enterprise_id = b.enterprise_id
       ...
       and c.month_new_mark = 0
       and a.ent_status_id = 0
       and a.enterprise_id not in (select enterprise_id from $dw_enterprise_msg_lmyyyymm  where is_test = 1)
     group by
           case when a.level_def_mode = 1 then '888'
                else value(a.ent_city_id,b.ent_city_id)
           end,
    ...
    select a.city_id,
     ...
     where a.member_income > 0
       and (a.ent_prod_income+a.ent_non_income) = 0
    ...
    select city_id,
           6,
           sum(case when a.s_index_id = 1 then result else 0 end) - sum(case when a.s_index_id = 5 then result else 0 end)
      from $stat_enterprise_0060_a_tmp5 a
     where a.s_index_id in (1,5)
     group by
             city_id"
    puts $sql_buf
    if [catch {aidb_sql $handle $sql_buf} errmsg] {
