...
#程序名称: stat_enterprise_0060_d.tcl
#程序功能: 2012年集团客户综合统计报表-4、单元产品
#分析目标: 略
#分析指标: 见维表
#分析维度: 全区 地市
#运行粒度: 月
#运行示例: crt_basetab.sh stat_enterprise_0060_d.tcl 2011-11-01
#创建时间: 2011-12-12
#创 建 人: Asiainfo-Linkage 
#存在问题:
#修改历史: 1、2011-7-12 修改集团订购口径，取消原rec_status = 1，改用生效、失效日期
#          2、2011-7-22 根据张阳工单（COM_OTHER_XZ_LS_20110708_49888 ），具体修改内容见工单附件 无线商话增加此统计口径.doc
#          3、2011-11-16 根据曾祥庆工单(COM_OTHER_XZ_LS_20111110_65259)修改增加无线商话数据
#          4、2012-7-5 根据熊成丞工单DEM_BASS_XZ_20120626_114507609修改无线商话口径，原口径已注释
#=======================================================================================
...
proc stat_enterprise_0060_d {p_optime} {
    ...
    ##集团订购产品付费使用情况临时表
    ####order_user_count 订购用户数（个人用户和集团成员）
    ####order_mem_count 集团成员订购数
    ####pay_user_count 付费用户数（个人用户和集团成员）
    ####pay_mem_count 付费成员数
    set sql_buf "
    ...
    ##无线商话临时表
    #根据熊成丞工单DEM_BASS_XZ_20120626_114507609修改口径，原口径已注释
#    set sql_buf "
...
#       and a.td_type_id = '4';"
   ...
   select a.city_id,
          case when a.service_id = '911' then
                    case when a.enterprise_name like '%幼儿园' then 125
                         when a.enterprise_name like '%小' or a.enterprise_name like '%小学' then 126
                         else 127
                    end
   ...
   select a.city_id,
          case when a.service_id = '911' then
                    case when a.enterprise_name like '%幼儿园' then 118
                         when a.enterprise_name like '%小' or a.enterprise_name like '%小学' then 119
                         else 120
                    end
...
#   }
    ...
    #Step6.根据张阳工单（COM_OTHER_XZ_LS_20110708_49888 ）修改结果表
...
##20120331 luodk add  与才德吉沟通，阿里医保通订购用户数。
    set sql_buf "
    insert into $stat_enterprise_0060_d_tmp4
    (
           city_id,
...
##20120331 luodk add  与才德吉沟通，将阿里医保通收入统计到该项中。
    set sql_buf "
    insert into $stat_enterprise_0060_d_tmp4
    (
           city_id,
