...
##						 	 filenum=`ls -l $task_id$scycleid*AVL |wc -l`
##						 	 cheknum=`awk -F} '{print $3}' ${task_id}${scycleid}000000.SUM  |sed s/"{"//`
##						 	 #判断文件个数是否完整
##						 	 if [ $filenum = $cheknum ] ; then
##						 	 		#合并文件
##						 	 		rm -f tmp_${task_id}${scycleid}000000.AVL
##						 	 		cat $task_id$scycleid*AVL >> tmp_${task_id}${scycleid}000000.AVL
##
##									#生成校验文件
...
##									mv $task_id$scycleid* ${BACKUP_PATH}/splitfile
##									mv tmp_${task_id}${scycleid}000000.AVL ${task_id}${scycleid}000000.AVL
##									mv tmp_${task_id}${scycleid}000000.CHK ${task_id}${scycleid}000000.CHK
##							 else
##							 		echo $task_id文件个数未完整!
##							  	continue
##							 fi
