...
#接口名称: 校园区域本网用户语音交往圈                                                               
#接口编码：22403                                                                                          
#接口说明：与校园区域本网用户发生语音通话行为的所有的对端号码集合。
#程序名称: G_I_22403_MONTH.tcl                                                                            
#功能描述: 生成22403的数据
#运行粒度: MONTH
#源    表：1.
#输入参数: void
...
#程序名
  global app_name
  set app_name "G_I_22403_MONTH.tcl"
        
  #删除本期数据
	set sql_buff "delete from bass1.G_I_22403_MONTH where time_id=$op_month"
  exec_sql $sql_buff
...
set sql_buff "ALTER TABLE G_I_22403_MONTH_2 ACTIVATE NOT LOGGED INITIALLY WITH EMPTY TABLE"
exec_sql $sql_buff


set sql_buff "
insert into G_I_22403_MONTH_2
select 
	$op_month
	,user_id
	,product_no
...
set sql_buff "delete from G_I_22403_MONTH_1 where op_time = $timestamp"
exec_sql $sql_buff

set sql_buff "
insert into G_I_22403_MONTH_1
		    	  select
				 $timestamp,
		    	  	 b.user_id,
		    	  	 b.product_no,
