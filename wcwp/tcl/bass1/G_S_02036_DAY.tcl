...
######################################################################################################		
#接口名称: WLAN自动认证资费套餐用户成功办理量                                                               
#接口编码：02036                                                                                          
#接口说明：上报当日所有成功办理的WLAN自动认证资费套餐的订购关系，包括申请立即生效和尚未生效的订购关系。套餐名称为“WLAN自动认证自费”，套餐档次为20，套餐自费编码为999912120590020001，套餐不限时长， 流量1G封顶
#程序名称: G_S_02036_DAY.tcl                                                                            
#功能描述: 生成02036的数据
#运行粒度: DAY
#源    表：1.
#输入参数: void
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
    ...
    puts $last_month
        ...
        #程序名
        global app_name
        set app_name "G_S_02036_DAY.tcl"
	
  #删除本期数据
	set sql_buff "delete from bass1.G_S_02036_DAY where time_id=$timestamp"
	exec_sql $sql_buff

  #直接来源于二经用户表数据，新的接口表
	set sql_buff "
			
		insert into bass1.G_S_02036_DAY
		(
				 TIME_ID
				,USER_ID
				,PKG_ID
		...
		select 
			...
			,replace(char(date(a.CREATE_DATE)),'-','') DEAL_DT
  ...
  #2.检查在网用户是否在用户表里头
	set sql_buff "select count(*) from 
	            (
		     select user_id from bass1.G_S_02036_DAY
