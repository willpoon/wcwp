...
#2011-03-31   panzhiwei	 G_S_04002_DAY_20100501bak，G_S_04018_DAY_20100501bak作废，取消相关取数代码
#2011-07-01   panzhiwei	 费用都改成按分上报！！
#######################################################################################################
...
##set last_last_month_day  "20100501"
    ...
    #清空临时表表3
    set handle [aidb_open $conn]
	set sql_buff "delete from bass1.g_s_22204_month_tmp3 where cycle_id='3'"
...
#2011-03-31 12:31:50
#1.去掉带 bass1.G_S_04002_DAY_20100501bak 的子句
#2.取消子查询
...
##	  "
    ...
    set handle [aidb_open $conn]
##去除带bass1.G_S_04018_DAY_20100501bak的子句  
#去除子查询    
##	set sql_buff "
...
#-----------------------------------------------------------------------------------
	...
	set sql_buff "
		...
		values('3',decimal(1.0*($RESULT_VAL1+$RESULT_VAL2-$RESULT_VAL3)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('3',decimal(1.0*($RESULT_VAL4+$RESULT_VAL5-$RESULT_VAL6)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('3',decimal(1.0*($RESULT_VAL7+$RESULT_VAL8)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('3',decimal(1.0*($RESULT_VAL9+$RESULT_VAL10)/1024/1024,20,2))
	  "
	...
	set sql_buff "
		...
		values('3',decimal(1.0*($RESULT_VAL11+$RESULT_VAL12+$RESULT_VAL13)/100,20,2))
	  "
    ...
    set handle [aidb_open $conn]
	set sql_buff "
		insert into bass1.g_s_22204_month
		  select
		     $op_month
		    ,'$op_month'
		    ,cycle_id
			,char(sum(value(td_cust,0)))
			,char(sum(value(td_mobile_cust,0)))
			,char(sum(value(td_datacard_cust,0)))
			,char(sum(value(td_wire_cust,0)))
			,char(sum(value(td_book_cust,0)))
			,char(sum(value(td_net_cust,0)))
			,char(sum(value(td_net_mobile_cust,0)))
			,char(sum(value(td_net_datacard_cust,0)))
			,char(sum(value(td_net_book_cust,0)))
			,char(sum(value(td_net_wire_cust,0)))
			,char(sum(value(td_term_cust,0)))
			,char(sum(value(td_term_mobile_cust,0)))
			,char(sum(value(td_term_datacard_cust,0)))
			,char(sum(value(td_phone_cust,0)))
			,char(bigint(sum(value(td_cust_dur,0))))
			,char(bigint(sum(value(td_cust_t_dur,0))))
			,char(bigint(sum(value(td_mobile_cust_dur,0))))
			,char(bigint(sum(value(td_mobile_cust_t_dur,0))))
			,char(bigint(sum(value(td_wire_cust_dur,0))))
			,char(bigint(sum(value(td_wire_cust_t_dur,0))))
			,char(bigint(sum(value(td_phone_cust_dur,0))))
			,char(bigint(sum(value(td_cust_flows,0))))
			,char(bigint(sum(value(td_cust_t_flows,0))))
			,char(bigint(sum(value(td_mobile_flows,0))))
			,char(bigint(sum(value(td_mobile_t_flows,0))))
			,char(bigint(sum(value(td_datacard_flows,0))))
			,char(bigint(sum(value(td_datacard_t_flows,0))))
			,char(bigint(sum(value(td_book_flows,0))))
			,char(bigint(sum(value(td_book_t_flows,0))))
			,char(bigint(sum(value(td_cust_fee*100,0))))
			,char(bigint(sum(value(td_mobile_cust_fee*100,0))))
			,char(bigint(sum(value(td_datacard_cust_fee*100,0))))
			,char(bigint(sum(value(td_book_cust_fee*100,0))))
			,char(bigint(sum(value(td_wire_cust_fee*100,0))))
		from bass1.g_s_22204_month_tmp3
	 group by cycle_id
	  "
