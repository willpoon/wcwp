...
#规则类型：日
#指标摘要：彩铃话单单价
#规则描述：F0：0.5 ≤ 本月彩铃话单费用/本月彩铃话单数 ≤ 3（元）
#校验对象：
#          BASS1.G_S_04015_DAY     彩铃话单
#输入参数:
#输出参数: 返回值:0 成功;-1 失败
#编 写 人：王琦
#编写时间：2007-03-22
...
####	aidb_close $handle
####	
####	#判断F0：0.5 ≤ 本月彩铃话单费用/本月彩铃话单数 ≤ 3（元）超标
####	if { $RESULT<0.5 || $RESULT>3.0 } {
####		set grade 2
####	        set alarmcontent "准确性指标F0超出集团考核范围"
####	        WriteAlarm $app_name $p_optime $grade ${alarmcontent}	
####	        
####        } elseif {$RESULT<=0.6 || $RESULT>=2.85 } {
####        	set grade 3
####	        set alarmcontent "准确性指标F0接近集团考核范围"
####	        WriteAlarm $app_name $p_optime $grade ${alarmcontent}
####	               	
####        }     
####}
