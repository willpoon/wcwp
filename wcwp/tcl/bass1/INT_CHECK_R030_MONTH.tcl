#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R030_MONTH.tcl
# ** 程序功能：一经校验程序（R030）
# ** 运行示例：int INT_CHECK_R030_MONTH.tcl
# ** 创建时间：2009-5-30 15:07:19
# ** 创 建 人：xufr
# ** 规则编号：R030
# ** 规则属性：业务逻辑
# ** 规则类型：月
# ** 指标摘要：综合账单中的用户标识
# ** 规则描述：综合账单中的用户标识都应该在用户表中存在
# ** 校验对象：03005 综合帐单
# **           02004 用户
# ** 输出参数：返回值:   0 成功; -1 失败
# ** 修改历史:
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R030_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R030'"
	puts ${sqlbuf}
	...
	#判断综合账单中的用户标识在用户表中不存在的记录数
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select count(*) from (
	select distinct user_id from bass1.g_s_03005_month where time_id = $op_month
	except
	...
	if { $RESULT_VAL1 > 0 } {
		set grade 2
		set alarmcontent "准确性指标R030超出集团考核范围"
