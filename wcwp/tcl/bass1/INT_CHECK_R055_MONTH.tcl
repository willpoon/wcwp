#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R055_MONTH.tcl
# ** 程序功能：一经校验程序（R055）
# ** 运行示例：int INT_CHECK_R055_MONTH.tcl
# ** 创建时间：2009-5-31 10:15:49
# ** 创 建 人：xufr
# ** 规则编号：R055
# ** 规则属性：业务逻辑
# ** 规则类型：月
# ** 指标摘要：SP业务代码接口中有效的SP企业代码
# ** 规则描述：SP业务代码表中有效的SP企业代码都应该在SP企业代码表中有效SP代码中存在
# ** 校验对象：06029 SP业务代码
# **           06012 SP企业代码
# ** 输出参数：返回值:   0 成功; -1 失败
# ** 修改历史:
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R055_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R055'"
	puts ${sqlbuf}
	...
	#SP业务代码表中有效的SP企业代码在SP企业代码表中不存在的记录数
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select count(*) from (
	select distinct sp_code from bass1.g_i_06029_month where time_id = $op_month
	except
	...
	if { $RESULT_VAL1 > 0 } {
		set grade 2
		set alarmcontent "准确性指标R055超出集团考核范围"
