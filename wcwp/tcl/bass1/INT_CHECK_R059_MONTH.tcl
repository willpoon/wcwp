#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R059_MONTH.tcl
# ** 程序功能：一经校验程序（R059）
# ** 运行示例：int INT_CHECK_R059_MONTH.tcl
# ** 创建时间：2009-5-30 10:38:56
# ** 创 建 人：xufr
# ** 规则编号：R059
# ** 规则属性：业务逻辑
# ** 规则类型：月
# ** 指标摘要：SP结算接口中信息费与应收劳务费用和应付结算费用的平衡关系
# ** 规则描述：信息费应大于等于应收劳务费用与应付结算费用之和
# ** 校验对象：05003 SP结算
# ** 输出参数：返回值:   0 成功; -1 失败
# ** 修改历史:
# **           修改日期       修改人        修改内容
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R059_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R059'"
	puts ${sqlbuf}
	...
	#信息费应小于应收劳务费用与应付结算费用之和的记录
	set handle [ aidb_open $conn ]
	set sqlbuf "
	select count(*) from bass1.g_s_05003_month where time_id = $op_month and int(pay_fee) + int(stlmnt_fee) > int(info_fee)"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	if { $RESULT_VAL1 > 0 } {
		set grade 2
		set alarmcontent "准确性指标R059超出集团考核范围"
