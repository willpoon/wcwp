#***********************************************************************************************************************************************
# ** 程序名称：INT_CHECK_R061_MONTH.tcl
# ** 程序功能：一经校验程序（R061）
# ** 运行示例：int INT_CHECK_R061_MONTH.tcl
...
proc Deal { op_time optime_month province_id redo_number trace_fd bass1_dir temp_data_dir semi_data_dir final_data_dir conn conn_ctl src_data obj_data final_data } {
	...
	#程序名称
	set app_name "INT_CHECK_R061_MONTH.tcl"

	#删除本期数据
	set handle [ aidb_open $conn ]
	set sqlbuf "delete from bass1.g_rule_check where time_id = $op_month and rule_code = 'R061'"
	puts ${sqlbuf}
	...
	set sqlbuf "
	select count(*) from (
		select sp_ent_code,count(*) from (
			select sp_ent_code,sp_bus_code from bass1.g_s_05003_month
			where time_id = $op_month
			group by sp_ent_code,sp_bus_code) a
		group by sp_ent_code
		having count(*) > 1
	) a"
    puts ${sqlbuf}
	if [ catch { aidb_sql $handle $sqlbuf } errmsg ] {
	...
	if { $RESULT_VAL1 > 0 } {
		set grade 2
		set alarmcontent "准确性指标R061超出集团考核范围"
