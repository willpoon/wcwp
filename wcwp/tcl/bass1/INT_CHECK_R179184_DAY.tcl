######################################################################################################
#程序名称：	INT_CHECK_R179184_DAY.tcl
#校验接口：	02061
#运行粒度: DAY
#输入参数: 
#输出参数: 返回值:0 成功;-1 失败
...
########################################################################################################3
	# R179			新增	月	02_集团客户	手机邮箱个人用户绑定关系接口中的用户标识存在于用户表中	"02004 用户
  #02061 手机邮箱个人用户绑定关系"	02061(手机邮箱个人用户绑定关系)中的"用户标识"都在02004(用户)的"用户标识"中	0.05	
...
#取割接后的数据
	...
	if {[format %.3f [expr ${RESULT_VAL} ]]>0 } {
		set grade 2
	        set alarmcontent " R179 校验不通过"
	        puts ${alarmcontent}	        
	        WriteAlarm $app_name $op_time $grade ${alarmcontent}
		 }

#R184			新增	月	02_集团客户	手机邮箱个人用户绑定关系接口中的集团客户标识存在于集团客户资料中	"01004 集团客户
#02061 手机邮箱个人用户绑定关系"	02061(手机邮箱个人用户绑定关系)中的"集团客户标识"都在01004(集团客户)的"集团客户标识"中	0.05	
