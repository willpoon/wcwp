...
#运行粒度: 月
#运行示例: ds App_xysc_school_user_ms.tcl 2010-09-01
#创建时间: 2010-11-18
...
#=======================================================================================
 ...
 proc Do_xysc_school_real_user_2 {p_optime} {
     ...
     "
	   ...
	   #5_1.大学/中职-语音(>=2000)(对端通话为当月校园通话占比大于60%中的客户的通话次数占比大于或等于80%,
	   #    或者通话人数占比大于或等于80%,且通话人数不少于20人及校园基站通话占比>=0.6的学校人数不少于2000)
	   set sql_buf "insert into bass2.xysc_school_real_user_2_tmp (
			   user_id
        ,product_no
        ,school_name
     ...
     "
	   ...
	   #5_2.大学/中职-短信(>=2000)(对端短信为当月校园通话占比大于60%中的客户的短信次数占比大于或等于80%,
	   #    或者短信人数占比大于或等于80%,且短信人数不少于20人及校园基站通话占比>=0.6的学校人数不少于2000)
	   set sql_buf "insert into bass2.xysc_school_real_user_2_tmp (
			   user_id
        ,product_no
        ,school_name
     ...
     "
	   ...
	   #5_3.大学/中职-语音(<2000)(对端通话为当月校园通话占比大于60%中的客户的通话次数占比大于或等于80%,
	   #    或者通话人数占比大于或等于80%,且通话人数不少于15人及校园基站通话占比>=0.6的学校人数少于2000)
	   set sql_buf "insert into bass2.xysc_school_real_user_2_tmp (
			   user_id
        ,product_no
        ,school_name
 ...
 proc Do_xysc_school_TeacherandStudent_user_1 {p_optime} {
     ...
     set sql_buf "drop TABLE bass2.xysc_school_teacherandstudent_oppcall2_$year$month"

	   if [catch {aidb_sql $handle $sql_buf} errmsg] {
	   	     puts "errmsg:$errmsg"
	   }
	   set sql_buf "CREATE TABLE bass2.xysc_school_teacherandstudent_oppcall2_$year$month like bass2.xysc_school_teacherandstudent_oppcall2_yyyymm in tbs_bass_miner
					        index in tbs_index partitioning key (user_id,product_no,opposite_no) using hashing not logged initially
     "
	   ...
	   set sql_buf "insert into xysc_school_teacherandstudent_oppcall2_$year$month
		     select
		    	  a.user_id,
		    	  a.product_no,
		    	  a.school_name,
	   ...
	   set sql_buf "create index idx_xysc_up11 on xysc_school_teacherandstudent_oppcall2_$year$month (user_id,product_no,school_name,opposite_no)"
