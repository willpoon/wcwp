...
#分析维度: 全区
#运行粒度: 月
#运行示例: crt_basetab.sh stat_market_0141.tcl 2011-06-01
#创建时间: 2011-6-03
...
proc deal {p_optime p_timestamp} {
    ...
    if {[stat_market_0141 $p_optime] != 0} {
        aidb_roll $conn
        aidb_close $handle
        return -1
    }
...
proc stat_market_0141 {p_optime} {
    global conn
    global handle
    ...
    #Step1.创建结果临时表
    set sql_buf "
    declare global temporary table session.stat_market_0141_mid
    (
			time_id          date,
			city_id          varchar(7),
			result1          integer,
    ...
    #3、其中存量客户办理量（单位个）
    set sql_buf "
    insert into session.stat_market_0141_mid
    (
				time_id
				,city_id
				,result1
		...
		from 
		   ...
		   select a.*
				...
				where a.offer_id=b.offer_id
				  and b.index=1
				  and date(a.create_date) between ${date_optime} and date('${last_month_date}')
	     ...
	     ,row_number() over(partition by a.product_instance_id order by a.create_date desc) row_id 
	        from bass2.dw_product_ord_cust_dm_${op_month} a,
				...
				where a.product_instance_id=e.user_id
				  ...
				  and date(b.create_date) between ${date_optime} and date('${last_month_date}')
				  and a.op_time between ${date_optime} and date('${last_month_date}')
				  and c.op_time between ${date_optime} and date('${last_month_date}')
				  --and date(c.valid_date) between ${date_optime} and date('${last_month_date}')
				  and c.state=1
				  and a.state=1
				  and d.index=1
				  and e.test_mark<>1
...
####生成全区的一条记录
 ...
 ;"
    ...
    select $date_optime,
           '890',
			...
			from 
				(
				select a.product_instance_id,a.customer_order_id,a.op_time,a.state,a.expire_date,a.valid_date,a.op_id,a.org_id
	     ,row_number() over(partition by a.product_instance_id order by a.create_date desc) row_id 
	        from bass2.dw_product_ord_cust_dm_${op_month} a,
				       bass2.dw_product_ins_off_ins_prod_$op_month b,
				       bass2.dw_product_ord_offer_dm_${op_month} c,
 ...
 ;"
	...
	# # step : 删除结果集当日数据
	# #===============================================================================================
		set sql_buf "delete from bass2.stat_market_0141 where time_id = ${date_optime}"
		puts ${sql_buf}
		if [ catch { aidb_sql $handle $sql_buf } errmsg ] {
	...
	##=====================================================
    set sql_buf "
    insert into bass2.stat_market_0141
    select 
				$date_optime
				,city_id
				,sum(value(result1,0))
		  ...
		  from session.stat_market_0141_mid
	group by city_id
     ;"
    puts $sql_buf
    if [catch {aidb_sql $handle $sql_buf} errmsg] {
  ...
  #Step.收集信息
	...
	exec db2 "runstats on table bass2.stat_market_0141 with distribution and detailed indexes all"
	exec db2 "terminate"
