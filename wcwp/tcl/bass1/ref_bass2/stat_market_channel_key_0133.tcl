#===================================================================================================#
#    名称： stat_market_channel_key_0133.tcl															                          #
#																									                                                  #
#    功能描述：2011年电子渠道关键数据统计月报表                           							 						#
#																									                                                  #
#    编写人：AsiaInfo lihongliang                                     		 日期:2011-04-07		      #
#===================================================================================================#
 proc deal {p_optime p_timestamp} {
 	   ...
 	   if {[stat_market_channel_key_0133 $p_optime]  != 0} {
 	   	  aidb_roll $conn
 	   	  aidb_close $handle
 	   	 return -1
 	   }
 ...
 proc stat_market_channel_key_0133 {p_optime} {
	 ...
	 puts  $lmonth
	  #取当前日期
      set sql_buf "Select current date - 1 days from dual"
	    puts $sql_buf
	   ...
	   set today [lindex [aidb_fetch $handle] 0]
	   puts $today
	   scan $today "%04s-%02s-%02s" t_year t_month t_day	
	   aidb_commit $conn
	   aidb_close $handle
    ...
    #201网站
       set sql_buf "insert into session.stat_market_channel_key_0133_tmp
                   ...
                   from 
                      (select product_instance_id
                       from dw_product_ord_cust_dm_$year$month
	  ...
	  #短信
	   set sql_buf "insert into session.stat_market_channel_key_0133_tmp
                ...
                from 
                        dw_kf_sms_cmd_receive_dm_$year$month a,
					             dw_product_$year$month b
                where a.cmd_id in (     select distinct(cmd_id) 
				                               from bass2.ODS_SYS_CMD_DEF_$t_year$t_month$t_day
   ...
   "
    ...
    set sql_buf "insert into session.stat_market_channel_key_0133_tmp
        select d.region_id,101,204,count(*)
         from dw_product_ord_cust_dm_$year$month a,
                      dw_product_ord_busi_dm_$year$month b,
                      DW_PRODUCT_ORD_BUSI_OTHER_$year$month  c,
					           dw_product_$year$month d
      ...
      group by case when brand_name like '%全球通%' then 1
        when brand_name like '%动感地带%' then 4 else 5 end, bill_id
            ...
            from (select b.parent_ob_id as busi_code_type,a.brand_id,
                   ...
                   from  $dw_ow_loading_log_yyyymm a, dim_sys_object5  b
...
where a.product_no=c.PRODUCT_NO and c.USER_ID=b.PRODUCT_INSTANCE_ID and a.busi_code_type     in (22)
     and c.userstatus_id in (1,2,3,6,8)
	  	   ...
	  	   set sql_buf "insert into session.stat_market_channel_key_0133_tmp
                     select a.region_id,105,
		                 ...
		                 from
                            ...
                            from  bass2.dw_product_$year$month a, bass2.dw_acct_payment_dm_$year$month b
	  ...
	  #202热线
	  	       ...
	  	       from dw_custsvc_cps_log_call_dm_$year$month a,
	  	            dw_product_$year$month b
	  ...
	  #203短信
	  	       ...
	  	       from  dw_kf_sms_cmd_receive_dm_$year$month a,
	  	            dw_product_$year$month b
	  ...
	  #205自助终端
         ...
         from (select distinct product_instance_id
               ...
               from dw_acct_payitem_dm_$year$month
   ...
   "
	  ...
	  #卡号销售量
	  	   set sql_buf "insert into session.stat_market_channel_key_0133_tmp
                    ...
                    from
                    bass2.dw_product_busi_dm_$year$month a,
                    bass2.dim_channel_info b,
                    bass2.dw_product_$year$month c
                    where a.so_org_id = b.channel_id
                  ...
                  select value(b.region_id,'89199'),103,
                            case when c.channel_type in (90153,90155,90157,90158,90196,90940,90942,90943,
                              90154,90941,90156) then 207
			                     ...
			                     count(*)
                     from dw_res_ctms_exchg_$year$month a
	  	  	   ...
	  	  	   set sql_buf "insert into session.stat_market_channel_key_0133_tmp
                        ...
                        from
                                  (select so_channel_id,paytype_id,sum(recv_cash) as recv_cash
                                   from $dw_acct_payitem_dm_yyyymm 
